...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    DisplayLockGuard lock(this);
    printf("[lcd_display] SetEmotion('%s')\n", emotion ? emotion : "(null)");
    if (emotion_label_ == nullptr) {
        return;
    }
...
bool valid_group = true;
    ...
    switch (name[0]) {
        ...
        case 'f':
            ...
            break;
        case 's':
            if (strcmp(name, "sad") == 0 || strcmp(name, "surprised") == 0) { group_index = 1; valid_group = true; }
            else if (strcmp(name, "shocked") == 0) { group_index = 0; valid_group = true; }
            break;
        case 'c':
            if (strcmp(name, "crying") == 0) { group_index = 1; valid_group = true; }
        ...
        case 't':
            ...
            break;
        case 'e':
            if (strcmp(name, "embarrassed") == 0) { group_index = 1; valid_group = true; }
            break;
    }

    // 播放 SD 动画，如果失败则回退
    if (valid_group) {
        printf("[lcd_display] 播放动画 group=%d\n", group_index);
        create_sd_video_anim_by_group(group_index, 5);
    } else {
        printf("[lcd_display] 播放失败: 无效的 group=%d\n", group_index);
    }
} else {
    printf("[lcd_display] 未匹配任何表情, 使用默认\n");
}
