...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (emotion_label_ == nullptr) {
        ...
        // 如果找到匹配的表情就播放对应动画，否则显示默认的neutral表情
    lv_obj_set_style_text_font(emotion_label_, fonts_.emoji_font, 0);

// 表情组映射逻辑
int group_index = 0;
bool valid_group = true;
create_sd_video_anim_by_group(group_index, 100);
    if (it != emotions.end()) {
    const char* name = it->text;
    switch (name[0]) {
        case 'h':
            if (strcmp(name, "happy") == 0) { group_index = 1; valid_group = true; }
            printf("第60行");
            break;
        case 'l':
            if (strcmp(name, "laughing") == 0) { group_index = 1; valid_group = true; }
            break;
        case 'f':
            if (strcmp(name, "funny") == 0) { group_index = 1; valid_group = true; }
            break;
        case 's':
            if (strcmp(name, "sad") == 0 || strcmp(name, "surprised") == 0) { group_index = 2; valid_group = true; }
            else if (strcmp(name, "shocked") == 0) { group_index = 0; valid_group = true; }
            break;
        case 'c':
            if (strcmp(name, "crying") == 0) { group_index = 2; valid_group = true; }
            else if (strcmp(name, "confused") == 0) { group_index = 1; valid_group = true; }
            break;
        case 'a':
            if (strcmp(name, "angry") == 0) { group_index = 0; valid_group = true; }
            break;
        case 't':
            if (strcmp(name, "thinking") == 0) { group_index = 1; valid_group = true; }
            break;
        case 'e':
            if (strcmp(name, "embarrassed") == 0) { group_index = 2; valid_group = true; }
            break;
    }

    // 播放 SD 动画，如果失败则回退
    if (valid_group && create_sd_video_anim_by_group(group_index, 10)) {
        printf("播放成功");
        lv_obj_t* img = lv_img_create(lv_scr_act());         // 1. 创建图像对象
        lv_obj_align(img, LV_ALIGN_CENTER, 0, 0);             // 2. 设置位置
        lv_img_set_src(img, "/sdcard/video/group0/out001.png");
        lv_obj_add_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN); // 播放成功，隐藏文字表情
    } else {
        printf("播放失败");
        create_sd_video_anim_by_group(group_index, 100);
        lv_label_set_text(emotion_label_, it->icon); // 播放失败，使用 emoji 表情
        lv_obj_clear_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
    }
} else {
    printf("未匹配");
    create_sd_video_anim_by_group(group_index, 10);
    lv_label_set_text(emotion_label_, "😶"); // 未匹配任何情绪，使用默认表情
    lv_obj_clear_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
}
